...
function OnTick()
    ...
    if SmiteTT.active and not myHero.dead and (CanUseSmite or CanUseQ) then
        ...
        if GolemL ~= nil then CheckMonster(GolemL) end
        if WolfL ~= nil then CheckMonster(WolfL) end
        if WraighL ~= nil then CheckMonster(WraighL) end
		if GolemR ~= nil then CheckMonster(GolemR) end
        if WolfR ~= nil then CheckMonster(WolfR) end
        if WraighR ~= nil then CheckMonster(WraighR) end
    end
end
    ...
    if object ~= nil and not object.dead and object.visible and object.x ~= nil then
		...
		elseif CanUseQ and DistanceMonster <=125+200 then
			if object.health <= qDamage then
					CastSpell(_Q, object)
			elseif CanUseSmite and object.health <= MixDamage then
...
function OnCreateObj(obj)
    ...
    if obj ~= nil and obj.type == "obj_AI_Minion" and obj.name ~= nil then
        ...
        elseif obj.name == "TT_NGolem5.1.1" then GolemR = obj end
    end
end
...
function OnDeleteObj(obj)
    ...
    if obj ~= nil and obj.name ~= nil then
        ...
        elseif obj.name == "TT_NGolem5.1.1" then GolemR = nil 				end
    end
end
